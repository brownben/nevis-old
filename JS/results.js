...
document.getElementById('results-pdf').addEventListener('click', function () {
    ...
    ipc.send('pdf-results-start', htmlHead + eventData.findOne().name + htmlHeadMiddlle + eventData.findOne().name + htmlHeaderNext + htmlMain + htmlFooter + new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '') + htmlFooterNext);
...
ipc.on('html-file', function (event, path) {
    ...
    fs.writeFileSync(path, htmlHead + eventData.findOne().name + htmlHeadMiddlle + eventData.findOne().name + htmlHeaderNext + htmlMain + htmlFooter + new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '') + htmlFooterNext, 'utf8');
})
